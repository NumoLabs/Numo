...
export function processVesuMarket(market: VesuMarket): ProcessedMarket {
	...
	const lstApr = (Number(market.stats.lstApr.value) / 10 ** market.stats.lstApr.decimals) * 100;
	...
	const reserve = Number(market.config.reserve.value) / 10 ** market.config.reserve.decimals;
	const totalCollateralShares = Number(market.config.totalCollateralShares.value) / 10 ** market.config.totalCollateralShares.decimals;
	const totalNominalDebt = Number(market.config.totalNominalDebt.value) / 10 ** market.config.totalNominalDebt.decimals;
